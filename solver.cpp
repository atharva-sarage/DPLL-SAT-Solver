 ...
 * indices of clauses which are UnitLiterals.
 */
class clauseSetCurrentState{
    public:
        vector <int> countClause; // stores count of unsatified literals
        vector <bool> isSatisfied; //whether a clause is satisfied
        vector <int> unitLiterals;  // stores literals of all unitLiterals for unitPropogation
...
class SATsolver{
    ...
    public: 
        ...
        /**
         * dpll function which takes argument. All arguments are passes as refrences.
        ...
        bool dpll(clauseSetCurrentState& state,vector<bool>&assigned,int satisfiedClauses){ 
            ...
            for(int i=0;i<state.unitLiterals.size();i++){
                ...
                if(clauseset->literalClauseMap[compUnitLiteral]->size()>0){  
                    ...
                    for(auto clauseNum:*(clauseset->literalClauseMap[compUnitLiteral])){
                         ...
                         * so we find the literal such that it and its complement is not assigned
             ...
             * This heuristic gave the best performance over literal satifying shortest clauses, first unasigned 
             * literal and randomly selecting literal.             * 
             */
            ...
            clauseSetCurrentState prototypeState=clauseSetCurrentState(state); 
            ...
            // remove the literal that was selected and instead 
